...
class AdminController extends Controller {
    ...
    public function statisticPayment() {
        ...
        $payment = Payment::all();
        $series = Series::all();
        $paymentSeries = PaymentsSeries::all();
        return view( 'admin.statistic.payment', compact( 'series', 'payment', 'payment2', 'movie', 'paymentSeries' ) );
    }
    ...
    public function sortPayment( Request $request ) {
        ...
        if ( $request->sortID && $request->sortIDYear ) {
            ...
            return view( 'admin.statistic.payment', compact( 'payment', 'payment2', 'movie', 'sortID', 'sortIDYear', 'paymentSeries', 'series' ) );
        } else if ( $request->sortID ) {
            $sortID = $request->sortID;
            $payment = Payment::whereMonth( 'created_at', $request->sortID )->get();
            return view( 'admin.statistic.payment', compact( 'payment', 'payment2', 'movie', 'sortID', 'paymentSeries', 'series' ) );
        } else if ( $request->sortIDYear ) {
            $sortIDYear = $request->sortIDYear;
            $payment = Payment::whereYear( 'created_at', $request->sortIDYear )->get();
            return view( 'admin.statistic.payment', compact( 'payment', 'payment2', 'movie', 'sortIDYear', 'paymentSeries', 'series' ) );
        } else {
            $payment = Payment::all();
            return view( 'admin.statistic.payment', compact( 'payment', 'payment2', 'movie', 'paymentSeries', 'series' ) );
        }
