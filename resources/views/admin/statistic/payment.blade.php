                    ...
                    <div class="m-0 font-weight-bold text-primary">
                        ...
                        <h4 class="float-right">Tổng tiền bán phim:
                            ...
                            @foreach ($payment as $item)
                                ...
                                @endforeach
                            @endforeach
                            @foreach ($paymentSeries as $item)
                                @foreach ($series as $sery)
                                    @if ($item->series_id == $sery->id)
                                        @php
                                            $total += $sery->price;
                                        @endphp
                                    @endif
                                            ...
                                            @if ($n != 0)
                                                <tr>
                                                    ...
                                                    <td><a class="text-decoration-none"
                                                            href="{{ route('user.movie', $item->movie_id) }}"
                                    ...
                                    @foreach ($series as $sery)
                                        @if ($item->series_id == $sery->id)
                                            <tr>
                                                <td>{{ $i++ }}</td>
                                                <td><a class="text-decoration-none"
                                                        href="{{ route('user.detailSeries', $item->series_id) }}"
                                                        title="Mở trong tab mới" target="_blank"><b>
                                                            {{ $sery->series_name }}
                                                        </b></a></td>
                                                <td>
                                                ...
                                                <td>
                                                    {{ number_format(round($sery->price)) . 'đ' }}
                                                </td>
                                                <td>
                                                    {{ number_format(round($sery->price) * $n) . 'đ' }}
                                                </td>
                                            </tr>
                                        @endif
                                    @endforeach
                                    ...
                                    <div class="col-md-4">
                                        <select class="form wide" name="sortID" data-toggle="movie-dropdown"
                                            ...
                                            @for ($i = 1; $i <= 12; $i++)
                                                @isset($sortID)
                                                    @if ($i == $sortID)
                                                        <option selected value="{{ $i }}">Tháng {{ $i }}
                                                        </option>
                                                        @php
                                                            $i++;
                                                        @endphp
