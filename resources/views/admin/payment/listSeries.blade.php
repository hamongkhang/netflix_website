...
@section('title', 'Danh Sách Mua Series Phim - MinMovie')
@section('content')
    <div id="content">
        ...
        <div class="container-fluid">
            ...
            <!-- Page Heading -->
            <h1 class="h3 mb-2 text-gray-800 text-center">QUẢN LÝ MUA SERIES PHIM</h1>
            <!-- DataTales Example -->
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <div class="m-0 font-weight-bold text-primary">
                        DANH SÁCH MUA SERIES PHIM
                    </div>
                </div>
                @if (session('thongbao'))
                        ...
                        <table class="table table-bordered table-hover table-striped text-center" id="dataTable"
                            ...
                            <tbody>
                                ...
                                @foreach ($payment as $item)
                                    @foreach ($series as $item3)
                                        @if ($item->series_id == $item3->id)
                                            <tr>
                                                <td>{{ $i++ }}</td>
                                                <td><a class="text-decoration-none"
                                                        href="{{ route('user.movie', $item->series_id) }}"
                                                    ...
                                                    @foreach ($user as $item2)
                                                        @if ($item2->id == $item->user_id)
                                                            <a class="text-decoration-none"
                                                                href="{{ route('admin.user.edit', $item2->id) }}"
